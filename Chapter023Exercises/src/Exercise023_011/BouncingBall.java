...
{
	private final int MAX_BALLS = 20; // maximum allowed balls
	private Circle[] balls; // array containing balls
	private Circle[] shadows; // array containing shadows
	...
	private class MouseHandler extends MouseAdapter implements MouseMotionListener
	{
		...
		public void mousePressed(MouseEvent mouseEvent)
		{
			...
			balls[ballCount % MAX_BALLS] = new Circle(x, y, 
					new Color (random.nextInt(255), random.nextInt(255), random.nextInt(255)), 10, 10);
			shadows[ballCount % MAX_BALLS] = new Circle (x, getSize().height - 6, Color.BLACK, 10, 6);
			ballCount ++;
...
class Circle
{
	...
	Circle(int x, int y, Color color, int width, int lenght)
	{
		this.x = x;
		this.y = y ;
		this.color = color;
	...
	{
		g.setColor(getColor());
		g.fillOval(getX(), getY(), width, lenght);
	}
