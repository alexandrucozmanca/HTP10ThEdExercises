...
// .1 added
// Remainder operation with code 34
// Exponential operation with code 35
// New line operation with code 50
// .2 added
// String input with code 12
// String output with code 13
// max memoryIndex method to dump memory only for used indexes
public class ModifiedSimpletronSimulation2 {
	...
	static void displayCode (int maxIndex)
	{
		System.out.printf("%n%-22s%n"
						+ "%-22s%05d%n"
						+ "%-22s  %03d%n"
	...
	static void executeInstructions()
	{
		...
		do
		{
			...
			switch(operationCode)
			{
				...
				case 12:
					System.out.printf("*** Command String Read. ***%n"
							+ "Please enter a desired String: ");

					String inputString = keyboardReader.nextLine();
					simpletron[operand] = inputString.length();

					for (int letter = 0; letter < inputString.length() ; letter++)
					{
						simpletron [operand + letter + 1] = (int) inputString.charAt(letter);
					}
					break;

				case 13:
					System.out.printf("*** Command String Write. ***%n");

					int length = simpletron[operand];
					String output = "";

					for (int letter = 1; letter <= length ; letter++)
					{
						output = output + (char) simpletron [operand + letter];
					}
	...
	{
		int maxIndex = 0;
		for (int index = 0; index < simpletron.length; index++)
			if (simpletron[index] != 0)
				maxIndex = index;
		return maxIndex;
	}
}
